...
*/
   ...
   if (abs($row['MACDHeight']) < $macdThershold) {
    ...
    // Step 2-1
   if ($row['emaConflict'] == '35R' && $row['MACDConvergence'] == 'Diver' && $row['ema3slopeDirection'] != 'Up') {
		   $thisAction = 'PUT'; 
   ...
   // Step 2-2
   if ($row['emaConflict'] == '35R' && $macdConver == 'Conver'
...
// เพิ่มเติมจาก Step-6 ใน Python 
   ...
   // Step 6-2
   if ($row['PreviousTurnType'] =='TurnDown' && $row['emaConflict'] =='35R' ) {
	   if ( $row['thisColor'] =='Green') {
				 $thisAction = 'CALL'; 
   ...
   // Step 6-4
   if ($row['PreviousTurnType'] =='TurnUp' && $row['emaConflict'] =='53G' ) {
	   if ( $row['thisColor'] =='Green') {
				 $thisAction = 'CALL'; 
   ...
   // Step 6-7
   if ($row['ema3slopeDirection'] =='Up' && $row['emaConflict'] =='35R' ) {
	   if ( $row['thisColor'] =='Green') {
				 $thisAction = 'CALL'; 
   ...
   // Step 6-8
   if ($row['ema3slopeDirection'] =='Down' && $row['emaConflict'] =='35R' ) {
	   if ( $row['thisColor'] =='Red') {
				 $thisAction = 'CALL'; 
   ...
   // Step 6-9
   if ($row['ema3slopeDirection'] =='Up' && $row['emaConflict'] =='53G' ) {
	   if ( $row['thisColor'] =='Green') {
				 $thisAction = 'CALL'; 
...
// Step 6-10
   ...
   // Step 6-12
   if ($row['PreviousTurnType'] =='N' && $row['PreviousTurnTypeBack2'] =='TurnUp' && $row['emaConflict'] =='35R' && abs($row['MACDHeight']*1000*1000) < 10 ) {
	   if ( $row['thisColor'] =='Red') {
				 $thisAction = 'CALL'; 
   ...
   // Step 6-13
   if ($row['PreviousTurnType'] =='TurnUp' && $row['PreviousTurnTypeBack2'] =='N' && $row['emaConflict'] =='53G' && $row['ema3slopeDirection']=='Up') {
	   if ( $row['thisColor'] =='Red') {
				 $thisAction = 'CALL'; 
   ...
   && abs($row['ema3SlopeValue']) > 20 ) {
    ...
    // Step 6-17
   if ($row['PreviousTurnType'] =='TurnUp' && $row['PreviousTurnTypeBack2'] =='TurnDown' && ($row['emaConflict']) == '53G' ) {
	   if ( $row['thisColor'] =='Red') {
				 $thisAction = 'CALL'; 
    ...
    // Step 6-17@
   if ($row['PreviousTurnType'] =='TurnUp' && $row['PreviousTurnTypeBack2'] =='TurnDown' && ($row['emaConflict']) == '53G' && $row['ema3slopeDirection'] =='Up') {
	   if ( $row['thisColor'] =='Green') {
				 $thisAction = 'CALL'; 
   ...
   if ($row['PreviousTurnType'] =='TurnUp' && $row['PreviousTurnTypeBack2'] =='TurnDown' 
	   && $row['PreviousTurnTypeBack3'] =='N' 
       && $row['PreviousTurnTypeBack4'] =='TurnUp' 
	   && ($row['emaConflict']) == '53G' && $row['ema3slopeDirection'] =='Up') {
	   if ( $row['thisColor'] =='Red') {
				 $thisAction = 'CALL'; 
    ...
    // Step 6-18
   if ($row['PreviousTurnType'] =='TurnUp' && $row['PreviousTurnTypeBack2'] =='N' && ($row['emaConflict']) == '53G' ) {
	   if ( $row['thisColor'] =='Green') {
				 $thisAction = 'CALL'; 
...
// Step 6-22
   ...
   && $row['emaConflict'] == '35R' && $row['ema3slopeDirection'] =='Up'
   ) {
	   if ( $row['thisColor'] =='Red') {
				 $thisAction = 'CALL'; 
   ...
   // Step 6-24
   if ($row['PreviousTurnType'] =='N' && $row['PreviousTurnTypeBack2'] =='N'
   && $row['ema3slopeDirection'] =='Down' && $row['emaConflict'] =='53G'
   ) {
	   if ( $row['thisColor'] =='Green') {
				 $thisAction = 'CALL'; 
   ...
   // Step 6-26-5
   if ($row['PreviousTurnType'] =='TurnDown' && $row['PreviousTurnTypeBack2'] =='N'
   && $row['ema3slopeDirection'] =='Down' && $row['emaConflict'] =='35R'
   ) {
	   if ( $row['thisColor'] =='Green') {
				 $thisAction = 'CALL'; 
...
function getActionFromIDVer2($row,$macdThershold) { 
...
//
// *******************   เริ่ม Case ตรงนี้   ************************
  //return  array($thisAction,$actionReason,$nextColor,$remark);
  if ($row['pip'] < 0.03) {
      $thisAction = 'Idle'; 	  
	  $actionReason = 'Code0-0';
      return  array($thisAction,$actionReason,$nextColor,$remark);
  }
...
// เพิ่มเติมจาก Step-6 ใน Python 
   ...
   if ($row['PreviousTurnType'] =='TurnUp' && 
       $row['PreviousTurnTypeBack2'] =='TurnDown' && 
	   $row['PreviousTurnTypeBack3'] =='N' && 
	   $row['ema3slopeDirection']=='Up' && $row['emaConflict']=='N' && $row['ema3SlopeValue'] <10 ) {
	   if ( $row['thisColor'] =='Green') {
				 $thisAction = 'CALL'; 
		         $actionReason .= '->Code6-5-3(G)';
		} else {
...
// Step 6-22
   ...
   && $row['PreviousTurnTypeBack4'] =='N'
   && $row['ema3slopeDirection'] =='Down' && $row['emaConflict'] =='35R'
   && $row['MACDConvergence'] =='Conver' && $row['MACDHeight'] < 0.1
   ) {
	   if ( $row['thisColor'] =='Red') {
				 $thisAction = 'CALL'; 
		         $actionReason .= '->Code6-26-8(G)';
...
function CalWinByID($id,$useExtra1=true,$useExtra2=true) { 
  ...
  $sql = "select * from $tableName WHERE candleID>= ? and candleID<=? order by candleID"; 
  $params = array($id,$id+30);

  $rs= pdogetMultiValue2($sql,$params,$this->pdo) ;
  if ($rs->rowCount() == 0) {
      return false;
  }
  ...
  $objTrade = new stdClass();
  $objTrade->MainID = $id ;
  

  $noteTrade = array();
  $totalTrade = 0 ; $tradeWin = False; 
 /* $useExtra1 = true; 
  $useExtra2 = true;
  */
  //$usecaseExtra1 = False; 
  ...
  while($row = $rs->fetch( PDO::FETCH_ASSOC )) {
    ...
    $row['ema3slopeDirection'] = $row['ema3slopeDirection'] ;     
	$timeframe = $row['timeframe'];
	$lastID = $row['candleID'];
	$totalTrade++ ;
    if ($vername == 'Ver2' ) {     
	  list($thisAction,$forecastColor,$forecastClass,$ActionClass,$actionReason,
	  $nextColorClass,$remark,$macd,$macdConver,$slopeValue,$slopeDirection,$pipSize,$delTapip) =$this->getActionFromIDVer2($this->pdo,$tableName,$row,$macdThershold,$lastMacdHeight);
	  ...
	  if ($useExtra1) {	  
		  ...
		  if ($thisAction2 !='') {
			...
			//echo '<h1>this Action = '. $row['id'] .' = ' . $forecastColor2  . '</h1><br>';
		  }
	  }
	  ...
	  if ($numLoss >= 3) {
         list($thisAction2,$forecastColor2,$forecastClass2,$ActionClass2,$actionReason2)= $this->useCaseLoss4Up($row) ;
		 if ($thisAction2 !='') {
			$thisAction = $thisAction2 ;
			$forecastColor = $forecastColor2 ;
			$actionReason .= '->'. $actionReason2 ;			
    ...
    $tmpTrade = new stdClass(); 
	$tmpTrade->tradeOnID = ' เทรดโดย ID = ' .$row['candleID'] ;
	$tmpTrade->tradeOnTimestamp =  $row['timefrom'] ;
	$tmpTrade->tradeOnTime =  $row['timefrom_unix'] ;
	 

	$tmpTrade->forecastColor =   $forecastColor ;
	...
	$sql = "select thisColor from $tableName where candleID=?"; 
	$nextID = $row['candleID']+1;
	$params = array($nextID);
	$resultColor =pdogetValue($sql,$params,$this->pdo) ;
	$tmpTrade->action = $actionReason;
	$tmpTrade->resultColor = 'ID = ' .$nextID. ' ผล Color คือ ->' .$resultColor ;
	$profit = 0 ;
	if ($thisAction != 'Idle') {
        $numTrade++ ;	
        ...
        $winStatus= 'Equal(Idle)' ;		
		$profit = 0 ;
	} 
	$tmpTrade->winStatus = $winStatus;
	$tmpTrade->numLoss = $numLoss;
	$noteTrade[] = $tmpTrade ;
	$remark = '';
	$curpair = 'EURUSD';
	...
	//echo '<hr>This ID =' . $row['candleID'] . ' FC Color= '.$forecastColor .' Next ID = ' . $nextID . '  Result Color = ' . $resultColor . ' WIn Status= '.$winStatus .'<br>';
	 

	if ($winStatus== 'win') {
		//echo 'WIN****';
		$objTrade->noteTrade = $noteTrade;
		$tradeWin = True;
		break ;
	} else {
       $no++ ; 
...
//echo ' Win AT ID ' . $lastID . '<br>';
//echo '<span style="color:red"> NumTrade = ' .$totalTrade . '</span><br>';
//print_r($noteTrade) ;
$objTrade->timeframe = $timeframe ; 
$objTrade->numLoss = $numLoss;
/*
...
*/
  ...
  return array($numTrade,$numLoss,$allActionReason,$balance,$timeframe,$objTrade);
  /*
  $myObj = new stdClass();
  $myObj->result = 'win' ;
...
emaConflict = '35R' AND slopeDirection='Down' 
...
function useCaseLoss4Up($row) { 
        ...
        $thisAction= 'CALL' ;
		$forecastColor = 'Green' ;
		$forecastClass = 'bgFreen' ;
		$ActionClass =  'bgGreen'; 
		$actionReason = 'CaseLosss4U-1'  ;
...
function getActionFromClsTradeDeriv($AnalyObj,$macdThershold,$lastActionCode) { 

require_once('clsTradeDeriv.php');
$clsTradeDeriv = new clsTradeDeriv();

   list($thisAction,$actionCode) =  $clsTradeDeriv->getAction($AnalyObj,$macdThershold);
...
function CalWinByIDVerObject($CandleID,$AnalystObject) {

         $pointerSelected = 0 ;  $timeCandle = '';
		// echo "Check->" . $CandleID ;
	     for ($i=0;$i<=count($AnalystObject)-1;$i++) {
	        if ($AnalystObject[$i]['candleID'] === $CandleID) {
               $pointerSelected = $i; 
			   $timeCandle =  $AnalystObject[$i]['timefrom_unix'] ;
			   break;
	        }
	     }  
		 $OverTradeList  = 'N'; 
		 $MoneyPlan = array(1,2,6,18,54,162,324,500,500,500,500,500,500,500,500);
		 $Percent = 0.90 ;
		 $numLoss = 0 ;

		 $macdThershold = 8 ; $win = false;
         $numcheck = 0 ;  $actionList = ''; $Balance = 0 ; $actionCodeList = '';
		 $numTrade= 0; $lastActionCode = '';
         while ($win === false) {
             //$numTrade++;
			// list($thisAction,$actionCode) = //$this->getActionFromIDVer2($AnalystObject[$pointerSelected],$macdThershold) ;
			 //if ($numLoss >= 4) {
                 list($thisActionDeriv,$actionCode) = $this->getActionFromClsTradeDeriv($AnalystObject[$pointerSelected],$macdThershold,$lastActionCode) ;  
				 if ($thisActionDeriv !='') {
                   $thisAction = $thisActionDeriv ;
				   $lastActionCode = $actionCode ;
				   $actionCode = '@#Deriv-' . $actionCode ;
				   
				 }
				 
			 //}
			 $actionList .=  $thisAction .',';			 
			 if ($pointerSelected+1 <= count($AnalystObject)-1) {			  
			    $nextColor  = $AnalystObject[$pointerSelected+1]['thisColor'] ;
				$pointerSelected++;
			 } else {
				 $nextColor  = '!!!' ; break;
			 }
			 ...
			 if ($thisAction != 'Idle') {			 
				 ...
				 if ($numTrade % 2 === 1) {
					$actionCodeList .= '<span style="color:blue">' .$numTrade.'</span>&nbsp;&nbsp;]' .$AnalystObject[$pointerSelected]['timefrom_unix']  
                    . ' Suggest = '. $suggestColor .
					'<span style="color:red">'.$actionCode .'</span>'. $winStatus
					. "($nextColor)"
					.',<br>';
				 } else {
				   $actionCodeList .=  '<span style="color:blue">'.$numTrade.'</span>]&nbsp;&nbsp;' . $AnalystObject[$pointerSelected]['timefrom_unix'] . ' ::: ' .$actionCode.
				   $winStatus	."(" . $nextColor . ")" .',<br>';
				 }
				 
				 if ($numcheck >= 12 ) {
					 $OverTradeList = $CandleID ;
					 break ;
				 }
			 } 
		 ...
		 $objTrade = new stdClass();
		 $objTrade->id = $CandleID ;

		 $objTrade->minute = $timeCandle ;
		 $objTrade->numcheck = $numTrade ;
		 $objTrade->OverTrade =  $OverTradeList ;
		 $objTrade->actionList = $actionList ;
		 $objTrade->actionCodeList = $actionCodeList ;		 
		 $objTrade->OverTradeList =  $OverTradeList ;

         //echo json_encode($objTrade); 
		 return $objTrade ;
